...
const ApplyLeave = () => {
    ...
    const {register,handleSubmit,reset,formState: { errors }} = useForm<LeaveFormData>({ resolver: zodResolver(leaveSchema) });
  ...
  const onSubmit = async (data: LeaveFormData) => {
    ...
    try {
        const response = await createLeave(backendUrl, data);
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
       ...
       <div>
        <label className="block mb-2"> Reason  </label>
         <textarea rows={4}  {...register("reason")} className="w-full rounded p-3 bg-[#161915] border border-[#3a5035] border border-[#3a5035]" />
