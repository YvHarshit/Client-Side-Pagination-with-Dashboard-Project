...
export const createLeave = async ( backendUrl: string, leaveData: LeaveFormData ) => {
  const { data } = await axios.post( `${backendUrl}/api/emp/create/leave`, leaveData, { withCredentials: true } );
  return data;
}; 


export const getMyLeaves = async (backendUrl: string) => {
    const { data } = await axios.get( `${backendUrl}/api/emp/get/leave`, { withCredentials: true });
    return data;
};
